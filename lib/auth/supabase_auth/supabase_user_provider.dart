...
class BizzBuzzSupabaseUser extends BaseAuthUser {
  BizzBuzzSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> bizzBuzzSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BizzBuzzSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
